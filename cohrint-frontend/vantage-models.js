/**
 * vantage-models.js — Shared AI model pricing & metadata
 * Single source of truth used by app.html and calculator.html
 * Updated: March 2026
 */

const VANTAGE_MODELS = [
  // ── OpenAI ──────────────────────────────────────────────────────────────────
...
function calcCost(model, promptTokens, completionTokens, cachedTokens = 0) {
  const m = VANTAGE_MODELS.find(x => x.name === model);
  if (!m) return 0;
...
if (typeof module !== "undefined") module.exports = { VANTAGE_MODELS, PROVIDER_META, calcCost, cheapestAlternative, allCosts };
