...
Usage:
  from logging_infra.metrics_collector import MetricsCollector

  mc = MetricsCollector("test_12_data_loading")
...
Data written to:
  tests/test-results/metrics.jsonl  — one JSON line per test run
  tests/test-results/metrics.html   — visual metrics dashboard

Developer notes:
  The MetricsCollector is NOT a replacement for the TestReporter.
  Use MetricsCollector for performance/latency data and TestReporter
  for pass/fail test results. They complement each other.

  To compare runs over time:
    jq '.endpoints[] | select(.error_rate > 5)' tests/test-results/metrics.jsonl
    jq '.summary.p95 | values' tests/test-results/metrics.jsonl | sort -n | tail -20
"""
