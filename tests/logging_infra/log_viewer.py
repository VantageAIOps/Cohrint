...
Usage:
  ...
  python tests/logging_infra/log_viewer.py --tail

  # Filter by level:
  python tests/logging_infra/log_viewer.py --level ERROR

  # Filter by logger name:
  python tests/logging_infra/log_viewer.py --logger test.auth_stability

  # Show only failed test assertions:
  python tests/logging_infra/log_viewer.py --level ERROR --since 1h

  # Count errors by logger:
  python tests/logging_infra/log_viewer.py --count

  # Show slowest requests:
  python tests/logging_infra/log_viewer.py --slowest 10

  # Parse from file:
  python tests/logging_infra/log_viewer.py --file tests/test-results/run.jsonl

Developer notes:
  Log lines are newline-delimited JSON (NDJSON / JSON Lines format).
  Set VANTAGE_LOG_FILE=tests/test-results/run.jsonl in your environment
  to capture all logs from a test run to a file.

  Then use this viewer to analyse:
    python tests/logging_infra/log_viewer.py --file tests/test-results/run.jsonl --level ERROR

  To stream live logs from production Cloudflare Workers:
    wrangler tail vantage-api --format json | python tests/logging_infra/log_viewer.py --stdin
"""
...
DEFAULT_LOG_FILE = Path(__file__).parent.parent / "test-results" / "run.jsonl"
