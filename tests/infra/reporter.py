...
Collects test results across all test modules and writes:
  ...
  2. A machine-readable JSON report  → artifacts/{run}/report.json
  3. A human-readable HTML report    → artifacts/{run}/report.html

Usage (called by run_suite.py after all tests finish):
  from infra.reporter import TestReporter

  r = TestReporter()
  r.record("test_01_api", "Signup with valid email", passed=True, duration_ms=320)
  r.record("test_02_ui", "Sign-in with wrong key",  passed=False,
...
class TestReporter:
    def __init__(self, run_label: Optional[str] = None, output_dir: Optional[Path] = None):
    ...
    # ── JSON report ──────────────────────────────────────────────────────────
    def write_json(self, path: Optional[Path] = None):
        out_path = path or (self._output_dir / "report.json")
    ...
    # ── HTML report ──────────────────────────────────────────────────────────
    def write_html(self, path: Optional[Path] = None):
        out_path = path or (self._output_dir / "report.html")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        rows = ""
        for e in self.entries:
            if e["skipped"]:
