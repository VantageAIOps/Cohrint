"""
Tests for cohrint SDK — run with: pytest tests/ -v
Uses the installed cohrint package (pip install cohrint).
"""
import pytest
import time
import sys
...
# ── Make sure the cohrint SDK is importable (installed via pip or local) ─
...
def test_find_cheapest():
    from cohrint.models.pricing import find_cheapest, calculate_cost
...
def test_event_efficiency_score():
    from cohrint.models.event import CohrintEvent, TokenUsage
    ev = CohrintEvent(
...
def test_queue_enqueue_and_flush(monkeypatch):
    ...
    q = EventQueue(api_key="crt_test_key", ingest_url="http://localhost:8000", debug=True)
    ev = CohrintEvent(event_id="q-1", timestamp=time.time(), org_id="test")
    q.enqueue(ev)
    q.flush_sync()
