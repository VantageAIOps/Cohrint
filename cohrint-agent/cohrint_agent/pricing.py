...
# cache_read  = read from prompt cache
# cache_write = write to prompt cache (Anthropic only; 0.0 for others)
MODEL_PRICES: dict[str, dict[str, float]] = {
    ...
    # Fallback for unknown models — billed at claude-sonnet-4-6 rates
    "default":              {"input": 3.00,  "output": 15.00, "cache": 0.30,  "cache_read": 0.30,  "cache_write": 3.75},
}
...
def calculate_cost(
    ...
    Unknown models fall back to the ``default`` rate table (currently
    ``claude-sonnet-4-6``) — returning 0.0 for an unknown model silently
    masks spend and can bypass budget checks (T-COST.unknown_model).
    """
    ...
    # Clamp negatives so a backend returning -1 tokens cannot produce a
    # negative cost that poisons the session accumulator.
