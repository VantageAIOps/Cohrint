...
export function printOptimization(result: OptimizationResult): void {
  ...
  console.log(
    color(
      `  Optimized: ${result.originalTokens} -> ${result.optimizedTokens} tokens (saved ${result.savedTokens}, -${pct}%)`
    )
  );
}
...
export function printCostSummary(cost: CostData, session: SessionSummary): void {
  const line = (label: string, value: string) => `  ${dim(label.padEnd(18))} ${value}`;
  console.log("");
...
export function createSpinner(message = "Thinking"): Spinner {
