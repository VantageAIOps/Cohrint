...
export type Category = "model" | "cache" | "prompt" | "workflow" | "budget";
...
interface FilledMetrics extends SessionMetrics {
  agent: string;
  model: string;
  avgCostPerPrompt: number;
  avgLatencyMs: number;
...
interface AgentTip {
  ...
  condition: (m: FilledMetrics) => boolean;
}
...
const AGENT_TIPS: AgentTip[] = [
  ...
  },
  {
    ...
    condition: (m) =>
      m.agent === "claude" && m.totalInputTokens > 50000 && m.promptCount > 5,
  },
  {
  ...
  },
  {
    ...
    condition: (m) =>
      m.agent === "claude" &&
      m.totalCachedTokens < m.totalInputTokens * 0.1 &&
      m.totalInputTokens > 20000,
  },
  // ── Gemini CLI ──
  {
    id: "gemini-use-flash",
  ...
  },
  {
    ...
    condition: (m) =>
      m.agent === "gemini" && m.totalCostUsd < 1 && m.promptCount < 50,
  },
  {
    ...
    condition: (m) =>
      m.agent === "gemini" &&
      m.totalInputTokens > 30000 &&
      m.totalCachedTokens < m.totalInputTokens * 0.2,
  },
  {
    id: "gemini-batch",
    priority: "low",
  ...
  },
  {
    ...
    condition: (m) =>
      m.agent === "codex" &&
      m.totalCachedTokens < m.totalInputTokens * 0.15,
  },
  // ── Aider ──
  {
    id: "aider-use-diff",
    ...
    condition: (m) =>
      m.agent === "aider" && m.totalOutputTokens > m.totalInputTokens * 2,
  },
  {
    id: "aider-deepseek",
    priority: "high",
...
export function getRecommendations(
  ...
  const durationMin =
    metrics.sessionDurationMin ??
    (metrics.sessionStartTime
      ? (Date.now() - metrics.sessionStartTime) / 60000
      : 0);
  const avgCost =
    metrics.avgCostPerPrompt ??
    (metrics.promptCount > 0 ? metrics.totalCostUsd / metrics.promptCount : 0);
  const agentName = normalizeAgentName(metrics.agent ?? "unknown");

  const filled: FilledMetrics = {
    ...metrics,
    agent: agentName,
  ...
  return applicable.slice(0, maxTips).map((tip) => ({
    ...
    action: tip.action
      ...
      .replace(
        "${pct}",
        Math.round(
          (filled.totalCachedTokens / Math.max(filled.totalInputTokens, 1)) * 100
        ).toString()
      ),
  }));
}
